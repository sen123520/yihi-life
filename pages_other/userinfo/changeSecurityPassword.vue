<template>
	<view>
		<view class="input-content">
			...
			<view class="input-item">
				<text class="tit">请输入新安全密码</text>
				<input type="password" v-model="password" placeholder="请输入8-18位必须包含数字+字母组合的密码" />
			</view>
			<view class="input-item">
				<text class="tit">请再次输入安全密码</text>
				<input type="password" v-model="passwordRe" placeholder="请再次输入密码以确保两次密码一致" />
			</view>
			<view class="input-item">
				<text class="tit">验证码</text>
				<input type="text" v-model="captcha" placeholder="请输入验证码" placeholder-style="font-size: 22rpx; color:rgba(50,50,51,0.6)" data-key="captcha" />
	...
	export default {
		...
		methods: {
			...
			async submit() {
				...
				try {
					const data = await this.$api.request('/user/edit', 'POST', {
