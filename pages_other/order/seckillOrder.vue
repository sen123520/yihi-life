<template>
	<view>
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b" v-if="pmode===1">
				<view class="cell-tit clamp upanddown">
					...
					<text class="up" style="font-weight: regular;">账户复消积分：<text style="color:#FF5040;"> {{ wallet.rebuyscore }} </text></text>
			...
			<view class="yt-list-cell b-b" v-if="pmode===2">
				<view class="cell-tit clamp upanddown">
					...
					<text class="up" style="font-weight: regular;">账户购物券：<text style="color:#FF5040;"> {{ wallet.greenscore }} </text> </text>
			...
			<view class="yt-list-cell b-b" v-if="pmode===2">
				<view class="cell-tit clamp upanddown">
					...
					<text class="up qian">(最多抵扣{{ deduction.max_greenscore }})</text>
					<text class="up" style="font-weight: regular;">账户福利购物券：<text style="color:#FF5040;"> {{ wallet.greenscore_benefit }} </text></text>
				</view>
				<view class="cell-tip">
		...
		<view class="footer">
			...
			<text class="submit" @click=" validateInputs() && submit()">提交订单</text>
		</view>
	...
	export default {
		...
		computed: {
			...
			calculateFinalPrice() {
				...
				// console.log(greenscore,greenscore_benefit,beenTotal,rebuyScoreTotal);
				let deductionTotal = greenscore + greenscore_benefit + beenTotal + rebuyScoreTotal
		...
		methods: {
			validateInputs() {
				...
				const inputsWithValue = [rebuy_score, greenscore, greenscore_benefit].filter(score => score > 0);

				if (inputsWithValue.length === 0) {
					uni.showToast({
						title: '请选择至少一种抵扣方式并填写数值！',
						duration: 2000,
						icon: 'none'
					});
					return false; // 没有任何输入大于0，阻止提交
				} else if (inputsWithValue.length > 1) {
					uni.showToast({
						title: '只能选择一种抵扣方式填写数值！',
						duration: 2000,
						icon: 'none'
					});
					return false; // 多于一个输入大于0，阻止提交
				}
				return true; // 只有一个输入大于0，允许提交
			},
			limitInput(field, e) {
			...
			async loadData() {
				...
				// if(e_info.user_info.jiaoyi_account==''){
				// 	return uni.showModal({
				// 		title:'未绑定交易账号',
				// 		content:'请前往绑定交易账号',
				// 		success:(res)=> {
				// 			if(res.confirm){
				// 				this.$api.navTo('/pages_other/userinfo/bindJiaoYi')
				// 			}else{
				// 				this.$api.navTo('/pages/product/list?fid=8')
				// 			}
				// 		}
				// 	})
				// }
				console.log('loadData', e_info);
				this.calcTotal();
			},
			async getUserScore() {
			...
			async getOrderCreate(param) {
				...
				console.log('秒杀类型', data.skill_pmode);
