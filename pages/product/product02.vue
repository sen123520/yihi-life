<template>
  <view class="container">
    ...
    <!-- 秒杀的话才显示 -->
    <!-- 		<view class="flash" v-if="category_id==8">
			...
			<view class="right">
				<view class="time" v-if="countdown && progress.number != progress.sold">
					{{product.flash.text}}
					<uni-countdown ref="countd" :showDay="countdown.day > 0 ? true : false" :day="countdown.day"
						:hour="countdown.hour" :minute="countdown.minute" :second="countdown.second" @timeup="timeup"
						color="#fffa30" borderWidth="22rpx" splitorColor="#fffa30" background-color="#282f2c00"
				...
				<view class="progress" v-if="progress.number">
					<ProgressBar class="ProgressBar" :Sold="progress.sold" :widthUpx="250"
						:Width="percentage(progress.number, progress.sold)" Type="candy" :Vice="true"></ProgressBar>
				</view>
			</view>
		</view> -->
    ...
    <view class="introduce-section" v-if="product.product_id">
      ...
      <text class="title up">{{ product.title }}</text>
      <view class="down">{{ product.subtitle }}</view>

      <view class="bot-row">
        <!-- <view class="bot-row-sales" v-if="(category_id == 1 || category_id == 4 || category_id == 5) && product.sales <= 0"></view> -->
      ...
      <!-- <view class="bot-row">
				<view>
					<text class="iconfont">&#xe605; 正品保证</text>
					<text class="iconfont">&#xe62F; 极速发货</text>
				</view>
			</view> -->
    </view>
    ...
    <view class="page-bottom">
      ...
      <view class="action-btn-group only_cart" v-if="!product.join_cart">
        <button type="primary" class="action-btn no-border buy-now-btn" @click="toggleSpec">{{ category_id == 8 ? '立即兑换' : '立即购买' }}</button>
      </view>
    </view>
...
.page-bottom {
  ...
  .action-btn-group {
    ...
    height: 100%;
    overflow: hidden;
    /* 			box-shadow: 1px 2px 5px rgba(0, 156, 234, 0.4);
			background: linear-gradient(to right, #42a9ae, #5ccacb, #45adb0); */
    margin-left: 20rpx;
    position: relative;
